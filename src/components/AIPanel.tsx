import { Check, ChevronLeft, ChevronRight, Clipboard, LoaderCircle, Sparkles, X } from 'lucide-react';
...
export function AIPanel({ collapsed, onToggle, status, summary, suggestions, onRun, onAccept, onReject, onAcceptAll, onRejectAll }: {
  collapsed: boolean; onToggle: () => void; status: 'idle' | 'running' | 'error'; summary: string;
  suggestions: AISuggestion[]; onRun: (task: AITask) => void; onAccept: (id: string) => void; onReject: (id: string) => void;
  ...
  return <aside className="ai-panel">
    ...
    <div className="suggestion-list">
      ...
      </article>) : <div className="ai-empty"><Sparkles /><p>选中文字，选择一种分析任务。AI 建议不会自动覆盖原文。</p></div>}
    </div>
  </aside>;
}
